...
const Calculator = ({ rates }) => {
  ...
  const swipeHandler = () => {
    ...
    setFieldValue('amount_received', formik.values.type === 'buy' ? formik.values.amount_sent / rates.sell : formik.values.amount_sent * rates.buy);
  };
  ...
  const amountChangeHandler = (e) => {
    ...
    if (formik.values.type === 'buy') setFieldValue(inputName, inputName === 'amount_received' ? +rawValue * rates.buy : +rawValue / rates.buy);
    if (formik.values.type === 'sell') setFieldValue(inputName, inputName === 'amount_received' ? +rawValue / rates.sell : +rawValue * rates.sell);
  };
